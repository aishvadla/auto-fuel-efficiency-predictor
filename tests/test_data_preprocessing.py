...
def test_data_preprocessing():
    ...
    assert len(X_val_arr) == len(y_val_arr), f"Validation features and target row count don't match"
    assert len(X_test_arr) == len(y_test_arr), f"Test features and target row count don't match"

    # No missing values after preprocessing
    assert not np.isnan(X_train_arr).any(), f"Training features contain NaN after preprocessing"
    assert not np.isnan(X_val_arr).any(), f"Validation features contain NaN after preprocessing"
    assert not np.isnan(X_test_arr).any(), f"Test features contain NaN after preprocessing"
    ...
    # Shape sanity check
    assert X_train_arr.shape[1] == X_val_arr.shape[1], f"Training and validation data feature count don't match"
